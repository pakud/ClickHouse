    ...
    while (tcp_server.isOpen())
    {
        ...
        try
        {
            ...
            */
            ...
            query_state->query_context->setClusterFunctionReadTaskCallback([this, &query_state]() -> ClusterFunctionReadTaskPtr
            {
                Stopwatch watch;
...
ClusterFunctionReadTaskPtr TCPHandler::receiveClusterFunctionReadTaskResponse(QueryState & state)
{
    UInt64 packet_type = 0;
    readVarUInt(packet_type, *in);
    ...
    switch (packet_type)
    {
        ...
        {
            auto task = std::make_shared<ClusterFunctionReadTask>();
            task->deserialize(*in);
            return task;
        }
